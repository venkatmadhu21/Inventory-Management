...
def update_employee(supid, field, new_value):
    try:
        cursor.execute("UPDATE requirement SET {}=? WHERE itemid=?".format(field), (new_value, supid))
        conn.commit()
        print("<p>requirement details updated successfully.</p>")
    except Exception as e:
        print("<p>Error updating requirement details: {}</p>".format(str(e)))

# Form to input employee details for update
print("<div style='text-align: center; padding: 20px;'>")
print("<form method='post' action=''>")
print("<label for='supid' style='color: #800000; font-weight: bold;'>Enter itemid :</label>")
...
print("<option value='empid'>empid</option>")
...
print("<option value='reqdate'>reqdate</option>")
