...
if "action" in form:
    ...
    if action == "Search" and "search" in form:
        search_query = form["search"].value
        cursor.execute("SELECT * FROM designation WHERE desid LIKE ? OR designation LIKE ? ", ('%' + search_query + '%', '%' + search_query + '%'))
...
<div style='text-align: center; padding: 20px;'>
    <form method='post' action=''>
        <label for='search' style='color: #800000; font-weight: bold;'>Search Designation by desid or designation:</label>
...
print("<th class='nav__title'>designation</th>")
