...
<div style='text-align: center; padding: 20px;'>
    <form method='post' action=''>
        ...
        <select id="sort" name="sort" style="margin-left: 10px; padding: 5px; border: 1px solid #800000;">
            ...
            <option value="depname">Department Name</option>
           
           
        </select>
        <button type=' ' name='action' value='Sort' style='padding: 5px 10px; background-color: #800000; color: white; border: none; cursor: pointer;'>Sort</button>
    </form>
</div>
""")
...
if "action" in form:
    ...
    if action == "Display":
        ...
        if employees:
            ...
            print("<tr><td colspan='14'>No department details found matching the search criteria.</td></tr>")
    elif action == "Sort" and "sort" in form:
        sort_query = form["sort"].value
        cursor.execute(f"SELECT * FROM department ORDER BY {sort_query}")
        employees = cursor.fetchall()
        for employee in employees:
            print("<tr>")
            for field in employee:
    ...
    print("<tr><td colspan='14'>No department detials found.</td></tr>")
