...
def delete_supplier(sid):
    try:
        cursor.execute("DELETE FROM signup WHERE empid=?", (sid,))
        conn.commit()
        print("<p>login details deleted successfully.</p>")
    except Exception as e:
        print("<p>Error deleting login details: {}</p>".format(str(e)))

# Form to input supplier ID for deletion
print("<div style='text-align: center; padding: 20px;'>")
print("<form method='post' action=''>")
print("<label for='sid' style='color: #800000; font-weight: bold;'>Enter empid ID to Delete:</label>")
...
if "action" in form and form["action"].value == "Delete":
    if "sid" in form:
        ...
        print("<p>No login ID provided for deletion.</p>")
