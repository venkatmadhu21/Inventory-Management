...
# Add some spacing between navbar and "Loginout Details" heading
...
if "action" in form:
    ...
    if action == "Display":
        ...
        cursor.execute("SELECT * FROM loginout WHERE empid LIKE ? OR logintime LIKE ? OR logouttime LIKE ? OR logdate LIKE ?", 
        ...
        if employees:
            ...
            print("<tr><td colspan='14'>No loginout found matching the search criteria.</td></tr>")
    elif action == "Sort" and "sort" in form:
        sort_query = form["sort"].value
        cursor.execute(f"SELECT * FROM loginout ORDER BY {sort_query}")
        employees = cursor.fetchall()
        for employee in employees:
            print("<tr>")
            for field in employee:
