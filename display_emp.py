...
if "action" in form and form["action"].value == "Display":
    cursor.execute("""SELECT 
        empid,
        fname || ' ' || COALESCE(mname, '') || ' ' || lname AS Name,
        desid,
        joiningdate,
        phoneno,
        dateofleaving,
        depid,
        emailid,
        area || ' ' || locality || ' ' || unitno || ' ' || Pincode AS Address,
        qualification 
    FROM employee""")
...
print("<div style='margin-top: 20px;width: 1000%;'></div>")
...
print("<th class='nav__title'>Name</th>")
