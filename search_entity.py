...
if "action" in form:
    ...
    if action == "Display":
        cursor.execute("SELECT * FROM rventity")
        suppliers = cursor.fetchall()
        display_suppliers(suppliers)

    elif action == "Search" and "search" in form:
        search_query = form["search"].value
        cursor.execute("SELECT * FROM rventity WHERE rvnumber LIKE ? OR itemid LIKE ? OR supid LIKE ? OR amount LIKE ?", ('%' + search_query + '%', '%' + search_query + '%', '%' + search_query + '%', '%' + search_query + '%'))
        suppliers = cursor.fetchall()
        display_suppliers(suppliers)
...
<div style='text-align: center; padding: 20px;'>
    <form method='post' action=''>
        <label for='search' style='color: #800000; font-weight: bold;'>Search itemid by ID or Name:</label>
...
print("<h1 style='color: #800000; font-size: 24px; font-weight: bold; text-align: left;'>reqiurement Details</h1>")
...
print("<th class='nav__title'>receiptdate</th>")
print("<th class='nav__title'>rvdate</th>")
