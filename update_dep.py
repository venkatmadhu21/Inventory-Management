...
def update_employee(supid, field, new_value):
    try:
        cursor.execute("UPDATE department SET {}=? WHERE depid=?".format(field), (new_value, supid))
        conn.commit()
        print("<p>department details updated successfully.</p>")
    except Exception as e:
        print("<p>Error updating department details: {}</p>".format(str(e)))

# Form to input employee details for update
print("<div style='text-align: center; padding: 20px;'>")
print("<form method='post' action=''>")
print("<label for='supid' style='color: #800000; font-weight: bold;'>Enter department ID:</label>")
...
print("<option value='depname'>Department Name</option>")
