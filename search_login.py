...
if "action" in form:
    ...
    if action == "Display":
        ...
        cursor.execute("SELECT * FROM signup WHERE empid LIKE ? OR username LIKE ?", ('%' + search_query + '%','%' + search_query + '%'))
        suppliers = cursor.fetchall()
...
<div style='text-align: center; padding: 20px;'>
    <form method='post' action=''>
        <label for='search' style='color: #800000; font-weight: bold;'>Search login by ID or Name:</label>
