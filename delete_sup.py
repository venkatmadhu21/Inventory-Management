...
def delete_supplier(supid):
    try:
        cursor.execute("DELETE FROM suppliers WHERE supid=?", (supid,))
        conn.commit()
        print("<p>Supplier details deleted successfully.</p>")
    except Exception as e:
        print("<p>Error deleting supplier details: {}</p>".format(str(e)))

# Form to input supplier ID for deletion
print("<div style='text-align: center; padding: 20px;'>")
print("<form method='post' action=''>")
print("<label for='supid' style='color: #800000; font-weight: bold;'>Enter Supplier ID to Delete:</label>")
...
if "action" in form and form["action"].value == "Delete":
    if "supid" in form:
        ...
        delete_supplier(supid)
    else:
        print("<p>No Supplier ID provided for deletion.</p>")
