...
<div style='text-align: center; padding: 20px;'>
    <form method='post' action=''>
        ...
        <select id="sort" name="sort" style="margin-left: 10px; padding: 5px; border: 1px solid #800000;">
            ...
            <option value="reqdate">reqdate</option>
...
print("<h1 style='color: #800000; font-size: 24px; font-weight: bold; text-align: left;'>Login Details</h1>")
...
if "action" in form:
    ...
    if action == "Display":
        ...
        cursor.execute("SELECT * FROM rvtable WHERE rvnumber LIKE ? OR itemid LIKE ? OR amount LIKE ? ", 
        ...
        if employees:
            ...
            print("<tr><td colspan='14'>No rv table found matching the search criteria.</td></tr>")
    elif action == "Sort" and "sort" in form:
        sort_query = form["sort"].value
        cursor.execute(f"SELECT * FROM requirement ORDER BY {sort_query}")
        employees = cursor.fetchall()
        for employee in employees:
            print("<tr>")
            for field in employee:
    ...
    print("<tr><td colspan='14'>No rv table found.</td></tr>")
