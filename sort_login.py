...
print("<th class='nav__title'>username</th>")
print("<th class='nav__title'>password</th>")
...
if "action" in form:
    ...
    if action == "Display":
        ...
        cursor.execute("SELECT * FROM signup WHERE empid LIKE ? OR username LIKE ? OR password LIKE ? ", 
        ...
        if employees:
            ...
            print("<tr><td colspan='14'>No login found matching the search criteria.</td></tr>")
    elif action == "Sort" and "sort" in form:
        sort_query = form["sort"].value
        cursor.execute(f"SELECT * FROM signup ORDER BY {sort_query}")
        employees = cursor.fetchall()
        for employee in employees:
            print("<tr>")
            for field in employee:
