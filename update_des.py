...
def update_employee(supid, field, new_value):
    try:
        cursor.execute("UPDATE designation SET {}=? WHERE desid=?".format(field), (new_value, supid))
        conn.commit()
        print("<p>designation details updated successfully.</p>")
    except Exception as e:
        print("<p>Error updating designation details: {}</p>".format(str(e)))

# Form to input employee details for update
print("<div style='text-align: center; padding: 20px;'>")
print("<form method='post' action=''>")
print("<label for='supid' style='color: #800000; font-weight: bold;'>Enter desid:</label>")
...
print("<option value='desid'>desid</option>")
print("<option value='designation'>designation</option>")
