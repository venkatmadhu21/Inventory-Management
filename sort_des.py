...
<div style='text-align: center; padding: 20px;'>
    <form method='post' action=''>
        ...
        <select id="sort" name="sort" style="margin-left: 10px; padding: 5px; border: 1px solid #800000;">
            <option value="desid">desid</option>
...
if "action" in form:
    ...
    if action == "Display":
        ...
        cursor.execute("SELECT * FROM designation WHERE desid LIKE ? OR designation LIKE ?", 
        ...
        cursor.execute(f"SELECT * FROM designation ORDER BY {sort_query}")
        employees = cursor.fetchall()
        for employee in employees:
            print("<tr>")
            for field in employee:
